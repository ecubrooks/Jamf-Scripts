...
# Last Updated: 2025-07-10, 2025-08-29
################################################################
...
install_latest_macos_update() { # $1: enforce update
    ...
    if [[ "$IS_APPLE_SILICON" == "yes" ]]; then
        ...
        if [[ $attempt -gt $max_attempts ]]; then
            ...
            --message "The macOS update failed due to too many incorrect password attempts. Please contact SoDM Informatics for assistance." \
            --mini \
            --infobuttontext "Need Help" \
            --infobuttonaction "$supportURL"
...
show_dialog() {
    ...
    --icon "$dialogIcon" \
    --message "For continued security and compatibility, your Apple computer requires an update to the latest version, **${LATEST_UPDATE}**.<br><br>You may defer the update for an additional **$remaining_deferrals** times before it becomes mandatory.<br><br>Thank you,<br>SoDM Informatics.<br>252-737-7100" \
